...
/**
 * on-skill-end.cjs
 * PostToolUse Hook (matcher: Skill|Agent) — 스킬/에이전트 완료 시 자동 실행
 *
 * 역할:
 * 1. 완료된 스킬/에이전트를 agent-status.json 배열에서 remove
 * 2. daily log에 에이전트 종료 + 소요 시간 기록
 */
...
process.stdin.on('end', () => {
  try {
    ...
    // agent-status.json — 배열에서 remove + 소요시간 계산
    let duration = '';
    let data = { agents: [] };
    ...
    // 이름이 매칭되는 첫 번째 에이전트를 찾아서 제거
    const idx = data.agents.findIndex(a => a.name === agentName);
    if (idx !== -1) {
      const removed = data.agents.splice(idx, 1)[0];
      if (removed.started) {
        try {
          const startTime = new Date(removed.started.replace(' ', 'T') + ':00+09:00');
          const elapsed = Math.round((now - startTime) / 1000);
          if (elapsed > 0 && elapsed < 7200) {
            const min = Math.floor(elapsed / 60);
            const sec = elapsed % 60;
            duration = min > 0 ? ` (${min}분 ${sec}초)` : ` (${sec}초)`;
          }
        } catch { }
      }
    }
    ...
    const row = `| ${timestamp} | 에이전트 종료 | ${displayName}${duration} |\n`;
    ...
    process.stderr.write('[Hook] on-skill-end error: ' + err.message + '\n');
  }
